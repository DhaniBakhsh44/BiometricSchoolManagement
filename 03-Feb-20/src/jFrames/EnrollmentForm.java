...
//package biometricClasses;

import com.digitalpersona.onetouch.*;
import com.digitalpersona.onetouch.processing.*;
import jFrames.TeacherRegisterFrame;
import java.awt.*;
...
public class EnrollmentForm extends CaptureForm
{
		...
		if (features != null) try
		{
			...
			switch(enroller.getTemplateStatus())
			{
				case TEMPLATE_STATUS_READY:	// report success and stop capturing
					stop();
					((TeacherRegisterFrame)getOwner()).setTemplate(enroller.getTemplate());
				...
				case TEMPLATE_STATUS_FAILED:	// report failure and restart capturing
					...
					stop();
					updateStatus();
					((TeacherRegisterFrame)getOwner()).setTemplate(null);
	...
	private void updateStatus()
	{
		...
		setStatus(String.format("Fingerprint samples needed: %1$s", enroller.getFeaturesNeeded()));
               JOptionPane.showMessageDialog(this,enroller.getFeaturesNeeded());
	}
